...
def load_dataset():
	...
	def load_15_lable_data_f32(filename):
		if not os.path.exists(filename):
			print ("Input data not exisits error")
			sys.exit()
		# Read the labels in Yann LeCun's binary format.
		...
		decimal = 100
		data = data * np.float32(100)
		print ( np.float32(data[:10]))
		#data = data.reshape(-1, 1,decimal)
		#print ('hey')
		# The labels are vectors of integers now, that's exactly what we want.
		return data
	...
	X_train = load_15_data('tonyu_data/standard/train_f.vector')
	#y_train = load_15_lable_data('tonyu_data/standard/train_f.label')
	y_train = load_15_lable_data_f32('tonyu_data/standard/train_f.label')
	
	#X_test = load_15_data('tonyu_data/standard/test_f.15')
	global test_file_name
	X_test = load_15_data('tonyu_data/standard/'+test_file_name)
	#y_test = load_15_lable_data('tonyu_data/standard/test_f.label')
	y_test = load_15_lable_data_f32('tonyu_data/standard/test_f.label')
...
def main(model='mlp', num_epochs=500):
	...
	print ("target_var : %s' " % target_var.dtype)
	...
	for epoch in range(num_epochs):
		...
		for batch in iterate_minibatches(X_train, y_train, batch_num, shuffle=True):
			...
			#targets = targets.reshape(-1,1,500)
			#print (inputs.shape)
			targets = targets.reshape(batch_num,1)
			####
			'''if train_batches > 10 and train_batches < 13:
				print ("targets")
				print (len(targets))
				print (targets[:5])
				pre_o = prediction_fn(inputs)
				print ("pre_o")
				print(len(pre_o))
				print (pre_o[:5])
				print ("shape")
				...
				#print (targets.shape)
				##
				loss_o = loss_fn(inputs,targets)
				print ("loss_o")
				print (loss_o)
			elif ( train_batches > 11):
				sys.exit()'''
			#sys.exit()
			############
	...
	test_acc = 0
	test_batches = 0
	for batch in iterate_minibatches(X_test, y_test, batch_num, shuffle=False):
		inputs, targets = batch
		targets = targets.reshape(batch_num,1)
		'''err, acc = val_fn(inputs, targets)
		test_err += err
